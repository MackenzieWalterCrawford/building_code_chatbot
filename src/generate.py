...
The LLM is forced to respond with structured output (OpenAI's Structured
Outputs / Anthropic's native structured output via messages.parse) matching
StructuredAnswer, rather than free-form prose. That removes the need to
regex citations or substring-match an "insufficient" phrase out of the
model's own wording — both citations and the sufficiency flag come back as
typed, schema-validated fields regardless of which provider answered.
